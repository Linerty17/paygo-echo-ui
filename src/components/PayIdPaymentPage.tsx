...
const PayIdPaymentPage: React.FC<PayIdPaymentPageProps> = ({ 
  ...
  const handleReceiptUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) setReceiptUploaded(true);
  };

  const handleConfirmTransfer = () => {
    if (!receiptUploaded) { alert('Please upload payment receipt'); return; }
    onTransferConfirmed();
  };

  return (
    <div className="min-h-screen min-h-[100dvh] bg-paygo-lavender w-full overflow-x-hidden">
      <div className="glass-header text-white p-4 w-full">
      ...
      <div className="p-4 space-y-6 w-full max-w-md mx-auto">
        ...
        <div className="glass-card border border-primary/20 rounded-xl p-4">
          <p className="text-foreground text-sm font-medium">You are purchasing a <strong>PAY ID</strong> for <strong>{userName}</strong> - Amount: <strong>{amount}</strong></p>
        </div>
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-primary/30 rounded-xl p-6 text-center glass-card">
            ...
            <label htmlFor="receipt-upload" className="cursor-pointer">
              {receiptUploaded ? (
                <div className="text-green-600"><div className="text-2xl mb-2">✓</div><p>Receipt uploaded successfully</p></div>
              ) : (
                <div className="text-muted-foreground"><div className="text-2xl mb-2">📄</div><p>Tap to upload payment receipt</p></div>
              )}
            </label>
          </div>
        </div>
